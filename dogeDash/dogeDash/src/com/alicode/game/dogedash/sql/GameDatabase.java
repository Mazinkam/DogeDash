...
public class GameDatabase {
	...
	private static String CREATE_LEVELONE_TABLE = "CREATE TABLE IF NOT EXISTS " + TABLE_LVL1 + "(" + KEY_ID + " INTEGER PRIMARY KEY," + KEY_SCORE
	...
	private void initDb() {
		...
		// deleteLevelHighscore(new Level(2), "levelDay");
		 Gdx.app.log(DogeDashCore.LOG, ""+getLevelHighscore(2, "levelDay").caughtPuppyNum);
...
//		}
	...
	// ----------------------------------------------------------------LVL DB
	public void addLevelHighscore(Level level, String dbName) {
		try {
			dbHandler.execSQL("INSERT INTO " + dbName + "  VALUES (" + level.getId() + "," + level.getHighScore() + "," + level.getStylePoints()
					+ "," + level.getTimeAlive() + "," + level.getCaughtPuppyNum() + "," + level.getMissedPuppyNum() + "," + level.getPuppyPoints()
					+ ")");
		} catch (SQLiteGdxException e) {
			e.printStackTrace();
		}
		Gdx.app.log(DogeDashCore.LOG, "wrote into " + dbName);
	}

	public Level getLevelHighscore(int id, String dbName) {
		DatabaseCursor cursor = null;

		try {
			cursor = dbHandler.rawQuery("SELECT " + KEY_ID + "," + KEY_SCORE + "," + KEY_STYLE + "," + KEY_TIME + "," + KEY_CAUGHTPUPPIES + ","
					+ KEY_MISSEDPUPPIES + "," + KEY_PUPPYPOINTS + "FROM " + dbName + " WHERE " + KEY_ID + "=" + "'" + id + "'");
	...
	public List<Level> getLevelList(String dbName) {
		List<Level> levelList = new ArrayList<Level>();
		DatabaseCursor cursor = null;

		try {
			cursor = dbHandler.rawQuery("SELECT  * FROM " + dbName);
		} catch (SQLiteGdxException e) {
			e.printStackTrace();
		}
	...
	public void updateLevelHighscore(Level level, String dbName) {
		DatabaseCursor cursor = null;
		...
		Gdx.app.log(DogeDashCore.LOG, "Trying to update " + id + dbName + score + style + time + caughtPups + missedPups + pupPoints);

		try {
			dbHandler.execSQL("UPDATE " + dbName + " SET" + score + style + time + caughtPups + missedPups + pupPoints + " WHERE" + id);
		} catch (SQLiteGdxException e) {
	...
	public void deleteLevelHighscore(Level level, String dbName) {
		String id = " " + KEY_ID + "=" + level.getId();

		try {
			dbHandler.execSQL("DELETE FROM " + dbName + " WHERE" + id);
		} catch (SQLiteGdxException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public List<Misc> getMiscList(String dbName) {
