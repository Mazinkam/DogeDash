...
public class GameDatabase {
	...
	public GameDatabase() {
			...
			Gdx.app.log(Consts.LOG, "DB initalizing");
			...
			initDb();
			Gdx.app.log(Consts.LOG, "DB initalized");

	}

	private void initDb() {

		Gdx.app.log(Consts.LOG, "DAY LEVEL LIST---------------------------------------");
		...
		if (dayLevelList.isEmpty()) {
			...
			for (Level obj : dayLevelList) {
				...
				Gdx.app.log(Consts.LOG, log);

			}
		}

		Gdx.app.log(Consts.LOG, "NIGHT LEVEL LIST---------------------------------------");
		...
		Gdx.app.log(Consts.LOG, "HEAD COSTUME LIST---------------------------------------");
		if (headCostumeList.isEmpty()) {
			addCostume(new Costume(1, "Dopelli Hat", 0, 30), "headTable");
			addCostume(new Costume(2, "Dogevil Horns", 0, 30), "headTable");
			...
			for (Costume obj : headCostumeList) {
				...
				String log = "Id: " + obj.getId() + " Name: " + obj.getItemName() + " , owned: " + obj.getIsOwned() + " , price: "
						+ obj.getItemPrice();
		...
		Gdx.app.log(Consts.LOG, "NOSE COSTUME LIST---------------------------------------");
		if (noseCostumeList.isEmpty()) {
			addCostume(new Costume(1, "Dogestache", 0, 30), "noseTable");
			addCostume(new Costume(2, "Doofus Nose", 0, 10), "noseTable");
		...
		Gdx.app.log(Consts.LOG, "EYES COSTUME LIST---------------------------------------");
		if (eyesCostumeList.isEmpty()) {
			addCostume(new Costume(1, "Dogespecs", 0, 40), "eyesTable");
			addCostume(new Costume(2, "Dogocle", 0, 30), "eyesTable");
			addCostume(new Costume(3, "Dogester Glasses", 0, 20), "eyesTable");
			addCostume(new Costume(4, "Dogebrow", 0, 10), "eyesTable");
		...
		Gdx.app.log(Consts.LOG, "BACK COMSTUME LIST---------------------------------------");
		...
		Gdx.app.log(Consts.LOG, "SETTINGS LIST---------------------------------------");
		...
		Gdx.app.log(Consts.LOG, "MISC ITEMS LIST---------------------------------------");
	...
	public void addLevelHighscore(Level level, String tableName) {
		try {
			dbHandler.execSQL("INSERT INTO " + tableName + "  VALUES (" + level.getId() + "," + level.getHighScore() + "," + level.getStylePoints()
					...
					+ ")");
			Gdx.app.log(Consts.LOG, "wrote into " + tableName);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed wrote into " + tableName);
		}
	...
	public Level getLevelHighscore(int id, String tableName) {
		...
		try {
			...
			Gdx.app.log(Consts.LOG, "get from " + tableName);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed to get from" + tableName);
		}
		if (cursor != null) {
			cursor.next();
	...
	public List<Level> getLevelList(String tableName) {
		...
		try {
			...
			Gdx.app.log(Consts.LOG, "getting list from " + tableName);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed to get list from" + tableName);
		}
		if (cursor != null) {
			while (cursor.next()) {
	...
	public void updateLevelHighscore(Level level, String tableName) {
		...
		Gdx.app.log(Consts.LOG, "Updating " + id + tableName + score + style + time + caughtPups + missedPups + pupPoints);
	...
	public void addCostume(Costume costume, String tableName) {
		try {
			...
			Gdx.app.log(Consts.LOG, "created " + tableName);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed to created " + tableName);
		}
		// Gdx.app.log(Consts.LOG, "created " + tableName);
	}
	...
	public void updateCostume(Costume costume, String tableName) {
		...
		Gdx.app.log(Consts.LOG, "Updating " + id + name + tableName + owned + price);
	...
	public void addSettings(Settings settings) {
		try {
			...
			Gdx.app.log(Consts.LOG, "wrote into " + TABLE_SETTINGS);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed to write " + TABLE_SETTINGS);
		}
		// Gdx.app.log(Consts.LOG, "wrote into " + TABLE_SETTINGS);
	}
	...
	public Settings getSettings(int id) {
		...
		try {
			...
			Gdx.app.log(Consts.LOG, "getting from  " + TABLE_SETTINGS);
		} catch (SQLiteGdxException e) {

			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed getting from  " + TABLE_SETTINGS);
		}
		if (cursor != null) {
			cursor.next();
	...
	public List<Settings> getSettingsList() {
		...
		try {
			...
			Gdx.app.log(Consts.LOG, "getting list  " + TABLE_SETTINGS);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed getting list " + TABLE_SETTINGS);
		}
		if (cursor != null)
			while (cursor.next()) {
	...
	public void updateSettings(Settings settings) {
		...
		Gdx.app.log(Consts.LOG, "Updating " + id + TABLE_SETTINGS + sound + music + vibration);
	...
	public void addMisc(Misc misc) {
		try {
			...
			Gdx.app.log(Consts.LOG, "write into " + TABLE_MISC);
		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed wrote into " + TABLE_MISC);

		}
		// Gdx.app.log(Consts.LOG, "wrote into " + TABLE_MISC);
	}
	...
	public List<Misc> getMiscList() {
		...
		try {
			...
			Gdx.app.log(Consts.LOG, "wrote list " + TABLE_MISC);

		} catch (SQLiteGdxException e) {
			// e.printStackTrace();
			Gdx.app.log(Consts.LOG, "failed write list" + TABLE_MISC);
		}
		if (cursor != null)
			while (cursor.next()) {
				Misc misc = new Misc();
	...
	public void updateMisc(Misc misc) {
		...
		Gdx.app.log(Consts.LOG, "Updating " + id + TABLE_MISC + coins);
	...
	public void dispose() {
		...
		dbHandler = null;
		Gdx.app.log(Consts.LOG, "dispose DB");
	}
