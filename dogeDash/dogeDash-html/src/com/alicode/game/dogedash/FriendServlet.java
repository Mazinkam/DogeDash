...
/**
 * FriendServlet adds the new logged in user if the user is not logged in
 * already Gives the list of users online The response is sent as an XML to
 * enable For each user the message that a new user logged is sent through the
 * channel API
 * 
 * @author
 * 
	...
	protected void doPost(HttpServletRequest request,
		...
		if (!friendStore.getFriends().contains(user)) {
			...
			logger.log(Level.INFO,
					"User {0} is already added  to list of users\n"
							+ "hence, not adding now", user);
		}

		Iterator<String> friendList = friendStore.getFriends().iterator();
		logger.log(Level.INFO,
				"All the users list is written to the output and "
		...
		while (friendList.hasNext()) {
			...
			if (!friend.equals(user)) {
				...
				channelService.sendMessage(new ChannelMessage(friend, "<data>"
						+ "<type>updateFriendList</type>" + "<message>" + user
